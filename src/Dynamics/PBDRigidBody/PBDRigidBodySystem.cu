...
namespace dyno
{
	IMPLEMENT_TCLASS(PBDRigidBodySystem, TDataType)

	typedef typename TOrientedBox3D<Real> Box3D;

	template<typename TDataType>
	PBDRigidBodySystem<TDataType>::PBDRigidBodySystem(std::string name)
		: Node(name)
	{
	...
	template<typename TDataType>
	PBDRigidBodySystem<TDataType>::~PBDRigidBodySystem()
	{
	}

	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::addBox(
		const BoxInfo& box,
	...
	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::addSphere(
	...
	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::addTet(
		const TetInfo& tet,
		const RigidBodyInfo& bodyDef, 
		const Real density /*= Real(1)*/)
	{
	...
	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::resetStates()
	{
	...
	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::updateTopology()
	{
	...
	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::getEulerAngle(float& yaw, float& pitch, float& roll)
	{
	...
	template<typename TDataType>
	void PBDRigidBodySystem<TDataType>::advect(float dt) {
		DArray<Vec3f> mm_velocity = stateVelocity()->getData();
	...
	DEFINE_CLASS(PBDRigidBodySystem);
}